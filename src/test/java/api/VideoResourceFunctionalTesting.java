...
public class VideoResourceFunctionalTesting {
	...
	public void testSendVideo() {
		...
		PlayerWrapper player = new RestBuilder<PlayerWrapper>(RestService.URL).path(Uris.PLAYERS).basicAuth(tokenValue, "").clazz(PlayerWrapper.class).get().build();
		assertEquals(0, player.getVideos().size());
		restService.sendVideo(tokenValue, new VideoWrapper("title1", "place1", "url1"));
		player = new RestBuilder<PlayerWrapper>(RestService.URL).path(Uris.PLAYERS).basicAuth(tokenValue, "").clazz(PlayerWrapper.class).get().build();
